...
$old_error_handler = set_error_handler("respaldo_app_enbc_Login_scriptcase_error_handler");

function respaldo_app_enbc_Login_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['respaldo_app_enbc_Login']['contr_erro']) && $_SESSION['scriptcase']['respaldo_app_enbc_Login']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_respaldo_app_enbc_Login->contr_respaldo_app_enbc_Login->Apl_com_erro = true;
            if ($inicial_respaldo_app_enbc_Login->contr_respaldo_app_enbc_Login->NM_ajax_flag)
            {
                $inicial_respaldo_app_enbc_Login->contr_respaldo_app_enbc_Login->NM_ajax_info['errList']['geral_respaldo_app_enbc_Login'][] = $msg;
                return;
            }
            respaldo_app_enbc_Login_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function respaldo_app_enbc_Login_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['respaldo_app_enbc_Login'])  && '' != $_SESSION['scriptcase']['error_icon']['respaldo_app_enbc_Login'])  ? $_SESSION['scriptcase']['error_icon']['respaldo_app_enbc_Login']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['respaldo_app_enbc_Login']) && '' != $_SESSION['scriptcase']['error_close']['respaldo_app_enbc_Login']) ? $_SESSION['scriptcase']['error_close']['respaldo_app_enbc_Login'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['respaldo_app_enbc_Login']) && '' != $_SESSION['scriptcase']['error_span_title']['respaldo_app_enbc_Login']) ? $_SESSION['scriptcase']['error_span_title']['respaldo_app_enbc_Login'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['respaldo_app_enbc_Login']) && '' != $_SESSION['scriptcase']['error_icon_title']['respaldo_app_enbc_Login']) ? $_SESSION['scriptcase']['error_icon_title']['respaldo_app_enbc_Login'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['respaldo_app_enbc_Login']['error_buffer'] .= ob_get_flush();
}
